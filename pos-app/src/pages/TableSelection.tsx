...
const TableSelection: React.FC = () => {
  ...
  const handleViewOrder = async (order: BuffetOrder) => {
    ...
    try {
      const { apiFetch } = await import('../config/api');
      const response = await apiFetch(`/api/orders/${order.id}/details`);
      if (response.ok) {
  ...
  const handlePrintBill = async () => {
    ...
    try {
      console.log('🖨️ [POS-APP] Bắt đầu in bill...');
      ...
      const billData = {
        ...
        tableName: orderDetails.table_name,
        area: orderDetails.area,
        staffName: employee?.fullname || 'N/A',
        customerName: 'Khách lẻ',
        items,
        buffetPackageName: orderDetails.buffet_package_name,
        buffetQuantity: orderDetails.buffet_quantity,
        buffetPrice: orderDetails.buffet_package_price || 0,
      ...
      drawText('HOA DON');
      drawText('====================');
      drawText(`${billData.tableName} - ${billData.area}`);
      if (billData.buffetPackageName) {
        drawText(`${billData.buffetPackageName} x${billData.buffetQuantity}`);
      }
      items.forEach(item => {
        drawText(`${item.name} x${item.quantity}`);
      });
      drawText('====================');
      drawText(`TONG: ${billData.totalAmount.toLocaleString()}d`);
