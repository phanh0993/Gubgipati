...
const BuffetPOS: React.FC = () => {
  ...
  const fetchPackages = async () => {
    try {
      ...
      const response = await apiFetch('/api/buffet-packages');
      if (response.ok) {
    ...
    const loadTableData = async () => {
      if (selectedTable) {
        try {
          ...
          const { apiFetch } = await import('../config/api');
          const response = await apiFetch(`/api/orders?table_id=${selectedTable.id}`);
          if (response.ok) {
  ...
  const fetchPackageItems = async (packageId: number) => {
    try {
      const { apiFetch } = await import('../config/api');
      const response = await apiFetch(`/api/buffet-package-items?package_id=${packageId}`);
      if (response.ok) {
        const data = await response.json();
  ...
  const handleCreateOrder = async () => {
    ...
    try {
      ...
      if (currentOrder) {
        ...
        const { apiFetch } = await import('../config/api');
        const response = await apiFetch(`/api/orders/${currentOrder.id}`, {
          method: 'PUT',
          headers: {
