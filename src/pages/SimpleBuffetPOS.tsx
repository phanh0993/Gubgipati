...
import {
  ...
  useTheme, useMediaQuery, TextField, Tabs, Tab, Chip
} from '@mui/material';
...
const SimpleBuffetPOS: React.FC = () => {
  ...
  const handleAddServiceItem = (serviceItem: any) => {
    ...
    if (isItemOutOfStock(serviceItem.id)) {
  ...
  const handleAddItem = (item: BuffetPackageItem) => {
    // Kiểm tra món có đang hết không
    if (isItemOutOfStock(item.food_item_id)) {
      alert('Món này đã hết, không thể order!');
      return;
    }
  ...
  // Function lấy employee_id - ưu tiên từ pos_employee trong localStorage (giống mobile)
  const getEmployeeId = async (userId?: number) => {
    try {
      // 1. Ưu tiên lấy từ pos_employee trong localStorage (giống mobile)
      const posEmployee = localStorage.getItem('pos_employee');
      if (posEmployee) {
        try {
          const employeeData = JSON.parse(posEmployee);
          if (employeeData && employeeData.id) {
            console.log('✅ [PC] Lấy employee_id từ pos_employee:', employeeData.id, employeeData.fullname || employeeData.full_name);
            return employeeData.id;
          }
        } catch (e) {
          console.warn('⚠️ [PC] Lỗi parse pos_employee:', e);
        }
      }
      
      // 2. Fallback: lấy từ user_id nếu có
      if (userId) {
        ...
        if (employee) {
          console.log('✅ [PC] Lấy employee_id từ user_id:', employee.id, employee.fullname || employee.full_name);
          return employee.id;
        }
      }
      
      // 3. Fallback cuối cùng: 14
      console.warn('⚠️ [PC] Không tìm thấy employee, dùng default: 14');
      return 14;
    } catch (error) {
      console.error('❌ [PC] Error resolving employee ID:', error);
      return 14;
    }
  };
  ...
  const handleCreateOrder = async () => {
    ...
    try {
      // Lấy employee_id - ưu tiên từ pos_employee, sau đó từ user_id
      ...
      if (currentOrder) {
        ...
        // Chỉ gửi items mới, API sẽ tự gộp với items cũ (cộng dồn số lượng)
        const updatedOrderData = {
          employee_id: employeeId,
          buffet_package_id: selectedPackage.id, // ✅ Thêm buffet_package_id để API biết đây là order thêm → cộng dồn
  ...
  const handlePrintBill = async () => {
    try {
      ...
      // Tạo bill data
      const billData = {
        orderNumber: currentOrder?.id ? `ORD-${currentOrder.id}` : `NEW-${Date.now()}`,
        tableName: selectedTable.table_name,
        area: selectedTable.area,
        staffName: currentOrder?.employee_name || 'N/A',
        customerName: 'Khách lẻ',
        items,
        buffetPackageName: selectedPackage.name,
        buffetQuantity: packageQuantity,
        buffetPrice: selectedPackage.price,
    ...
    <Box sx={{ height: '100vh', display: 'flex', flexDirection: 'column', bgcolor: '#f5f5f5' }}>
      ...
      <Box sx={{ display: 'flex', flex: 1, gap: 1, p: 1, height: 'calc(100vh - 80px)' }}>
                    ...
                    return serviceTypes.map((type) => (
                      <ListItem
                        ...
                        sx={{ 
                          ...
                          mb: 1, 
                          borderRadius: 1,
                          bgcolor: selectedServiceCategory === type ? 'primary.light' : 'transparent',
                          '&:hover': {
                            bgcolor: selectedServiceCategory === type ? 'primary.light' : 'grey.100'
                          }
                        }}
                      >
                        <ListItemText
        ...
        <Box sx={{ width: '50%', minWidth: '400px' }}>
          <Card sx={{ height: '100%' }}>
            <CardContent sx={{ height: '100%', display: 'flex', flexDirection: 'column', p: 1 }}>
              <Typography variant="h6" gutterBottom sx={{ textAlign: 'center', fontWeight: 'bold', mb: 1 }}>
                Khách lẻ
              </Typography>
              
              {/* Filter Categories - Chỉ hiển thị khi không phải serviceMode */}
              {!serviceMode && (
              ...
              {serviceMode ? (
                // Hiển thị danh sách món dịch vụ theo đầu mục được chọn
                        ...
                        <Grid item xs={6} sm={4} key={item.id}>
                          <Card 
                            sx={{ 
                              ...
                              border: 1,
                              borderColor: isOutOfStock ? 'grey.400' : 'grey.300',
                              opacity: isOutOfStock ? 0.5 : 1,
                              bgcolor: isOutOfStock ? 'grey.100' : 'white',
                              '&:hover': { 
                          ...
                          >
                            <CardActionArea>
                              <CardMedia
                                component="img"
                                height="80"
                                image={item.image_url || `https://via.placeholder.com/80x80/1976d2/FFFFFF?text=${encodeURIComponent(item.name.charAt(0))}`}
                                alt={item.name}
                                sx={{ objectFit: 'cover' }}
                              />
                              <CardContent sx={{ p: 1 }}>
                                <Typography variant="body2" sx={{ fontWeight: 'bold', mb: 0.5 }} noWrap>
                                  {formatFoodItemName(item.name)}
                                </Typography>
                  ...
                  <Grid container spacing={1}>
                    {packageItems
                      .filter((item) => {
                        ...
                        const itemType = (item.food_item as any).type || '';
                        // Also check category field if exists
                      ...
                      .map((item) => {
                        const isOutOfStock = isItemOutOfStock(item.food_item_id);
                        return (
                      <Grid item xs={6} sm={4} key={item.id}>
                        <Card 
                          sx={{ 
                          ...
                          <CardActionArea>
                            ...
                            <CardContent sx={{ p: 1, textAlign: 'center' }}>
                              <Typography variant="body2" fontWeight="bold" noWrap>
                                {formatFoodItemName(item.food_item.name)}
                              </Typography>
                            </CardContent>
                          </CardActionArea>
                        </Card>
                ...
                <Box sx={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <Typography color="text.secondary" variant="h6">
                    {serviceMode ? 'Vui lòng chọn đầu mục dịch vụ' : 'Vui lòng chọn gói vé trước'}
                  </Typography>
                </Box>
              )}
            </CardContent>
                  ...
                  {orderItems.map((item) => (
                    <ListItem key={item.food_item_id} sx={{ py: 0.5, px: 0, flexDirection: 'column', alignItems: 'stretch' }}>
                      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 1 }}>
                        <ListItemText
                          primary={formatFoodItemName(item.food_item.name)}
                          secondary={
                            <Box component="span">
