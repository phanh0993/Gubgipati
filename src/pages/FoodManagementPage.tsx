...
interface FoodItem {
  ...
  description: string;
  category_id: number;
  type: 'main' | 'side' | 'combo' | 'topping' | 'drink' | 'lau' | 'dessert';
  price: number;
  cost: number;
...
const FoodManagementPage: React.FC = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    description: '',
    category_id: 0,
    type: 'main' as 'main' | 'side' | 'combo' | 'topping' | 'drink' | 'lau' | 'dessert',
    price: 0,
    cost: 0,
    preparation_time: 15,
    is_available: true,
  ...
  // Thêm category mới cho Lẩu và Tráng miệng
    ...
    <Box sx={{ p: 3 }}>
      ...
      <Grid container spacing={3}>
        {Object.entries(groupedFoodItems).map(([type, items]) => {
          const categoryNames = ['all', 'Bò', 'Heo', 'Gà', 'Hải sản', 'Món nóng', 'Lẩu', 'Tráng miệng'];
          if (tabValue > 0) {
            const selectedCategory = categoryNames[tabValue];
            const expectedType = categoryToType[selectedCategory];
