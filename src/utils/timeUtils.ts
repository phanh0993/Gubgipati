 ...
 * Cộng thêm 7 giờ vào thời gian hiện tại để lưu vào database
 * @returns ISO string (UTC format, nhưng đã cộng thêm 7 giờ)
 */
export const getVietnamTimeForDB = (): string => {
  const now = new Date();
  // Cộng thêm 7 giờ (7 * 60 * 60 * 1000 milliseconds) để chuyển sang múi giờ Việt Nam
  const vietnamTime = new Date(now.getTime() + (7 * 60 * 60 * 1000));
  // Trả về ISO string (database sẽ lưu dưới dạng UTC, nhưng giá trị đã được cộng thêm 7 giờ)
  return vietnamTime.toISOString();
};
