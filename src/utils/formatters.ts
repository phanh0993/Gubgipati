...
// Format food item name - thay thế "COMBO NƯỚC + TRÁNG MIỆNG" thành "NƯỚC+TM"
export const formatFoodItemName = (name: string): string => {
  if (!name) return '';
  // Thay thế "COMBO NƯỚC + TRÁNG MIỆNG" (có thể có nhiều biến thể) thành "NƯỚC+TM"
  return name
    .replace(/COMBO\s*NƯỚC\s*\+\s*TRÁNG\s*MIỆNG/gi, 'NƯỚC+TM')
    .replace(/COMBO\s*NƯỚC\s*\+.*TRÁNG\s*MIỆNG/gi, 'NƯỚC+TM')
    .replace(/COMBO.*NƯỚC.*\+.*TRÁNG.*MIỆNG/gi, 'NƯỚC+TM');
};
